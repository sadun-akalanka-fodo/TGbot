...
# --- YouTube Handler (WITH COOKIES FROM cookies.txt) ---
async def handle_youtube(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str):
    ...
    cookies_path = "cookies.txt"  # In same folder as bot.py
    ydl_opts = {
    ...
    if os.path.exists(cookies_path):
        ...
        await msg.edit_text("Warning: cookies.txt not found. Limited access.")
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            temp_path = DATA_DIR / f"yt_{update.effect,update.effective_user.id}_{int(datetime.now().timestamp())}.mp4"
        ...
        if "Sign in" in error_msg or "bot" in error_msg.lower():
            await msg.edit_text("Warning: YouTube blocked download. Update cookies.txt (log in to YouTube first).")
        else:
...
# --- [ALL OTHER FUNCTIONS: cloud, inspector, rules, etc. — SAME AS BEFORE] ---
# ... (Copy from previous full version: main_menu, downloader_menu, callback_handler, etc.)
