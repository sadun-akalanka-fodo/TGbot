...
# CHANGED: real Telegram bot upload limit (~50 MB via api.telegram.org)
MAX_FILE_SIZE = 50 * 1024 * 1024      # 50 MB hard cap for bots
...
# --- UPLOAD AS DOCUMENT (50 MB BOT LIMIT SAFE) ---
async def upload_as_document(update: Update, context: ContextTypes.DEFAULT_TYPE, file_path: Path, caption: str = "", filename: str = None):
    ...
    # CHANGED: enforce 50 MB bot upload limit BEFORE sending
    if file_size > MAX_FILE_SIZE:
        await update.message.reply_text(
            f"❌ File is too large for Telegram bot uploads.\n"
            f"Size: {format_size(file_size)}\n"
            f"Limit: {format_size(MAX_FILE_SIZE)} (~50 MB)."
        )
        try:
            os.remove(file_path)
        except:
...
async def handle_youtube(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # CHANGED: check against 50 MB limit, not 2 GB
            if filesize and filesize > MAX_FILE_SIZE:
                await msg.edit_text(
                    f"❌ File appears too big for bot upload.\n"
                    f"Estimated size: {format_size(filesize)}\n"
                    f"Limit: {format_size(MAX_FILE_SIZE)} (~50 MB)."
                )
                return
...
async def handle_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with context.application.bot_data["session"].head(url) as resp:
            ...
            # CHANGED: enforce real bot limit
            if size and size > MAX_FILE_SIZE:
                await msg.edit_text(
                    f"❌ File is too big for Telegram bot uploads.\n"
    ...
    try:
        ...
        # extra safety: enforce limit after download too
        if file_size > MAX_FILE_SIZE:
            await msg.edit_text(
                f"❌ Downloaded file is too big for bot uploads.\n"
                f"Size: {format_size(file_size)}\n"
                f"Limit: {format_size(MAX_FILE_SIZE)} (~50 MB)."
            )
            try:
...
async def handle_file_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user["files"].append({
        "file_id": file_id,
        "file_name": file_name,
        "file_type": getattr(file, "mime_type", "unknown") or "unknown",
    ...
    try:
        ...
        if path.suffix.lower() in {'.jpg', '.jpeg', '.png', '.webp'}:
            with Image.open(path) as img:
                metadata = {"Format": img.format, "Resolution": f"{img.width}x{img.height}", "Size": format_size(getattr(file, "file_size", 0) or 0)}
                exif = img.getexif()
                if exif:
                    for tag_id in exif:
...
async def downloader_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "*Downloader (Bot Limit ~50 MB)*\n\n"
        "Send:\n"
        "• Direct file link\n"
        "• YouTube link\n\n"
        "⚠️ Files must be ≤ 50 MB for bot uploads.\n"
        "Files > 50 MB cannot be sent by this bot."
    )
    await update.callback_query.edit_message_text(
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif d.startswith("cloud_list_"):
        await cloud_list(update, context, int(d.split("_")[-1]))
    elif d.startswith("cloud_resend_"):
        await cloud_resend(update, context, int(d.split("_")[-1]))
    elif d == "cloud_wipe_confirm":
        await cloud_wipe_confirm(update, context)
    elif d == "cloud_wipe_yes":
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.document:
        ...
        for r in rules:
            if r["keyword"] in fn:
                try:
                    await update.message.forward(r["chat_id"])
                except:
                    pass
    ...
    elif state == "awaiting_file_save":
        await handle_file_save(update, context)
    elif state == "awaiting_inspector_file":
        await handle_inspector_file(update, context)
    elif state == "awaiting_rule":
